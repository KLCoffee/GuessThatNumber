...
while (restartGame){
    ...
    // Prompts user to enter a number of attempts allowed (AKA number of guesses) Also attempting to convert response to number value.
...
// Asks user to enter a guess in the range that they set
    ...
    // Checks if the used guessed correctly.  If so, the game ends (NOTE: The Break ends the loop).
    if (guess === randomNum){
        alert (`CONGRATULATIONS YOU GUESSED THE CORRECT NUMBER : 
        ${randomNum}`);
        break;

        // Checks if user has any attempts left. If not, the game ends and the number dispalys to the user.
    }   else if (attempts === 0){
...
//  Prompts user with option to play again
    ...
    // Loope continues until user submits a valid response
    while (true){
        // Checks if the user's response is no (AKA "N" or "n")
        if (playAgain.toUpperCase() === "N") {
            // Alerts the useer the game is over and the game/loop does not restart
            ...
            // Prompts the user to enter a vailid response and start this loop again
        } else {
